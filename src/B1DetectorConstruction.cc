...
#include "B1ElectricField.hh" //Magnetic <-> Electric
...
G4ThreadLocal B1ElectricField* B1DetectorConstruction::fElectricField = 0;
...
B1DetectorConstruction::B1DetectorConstruction(std::string sipmModelName, std::string housingName) 
...
G4SipmModel* B1DetectorConstruction::createSipmModel(std::string name) const {
	if (name == "generic") {
...
G4SipmHousing* B1DetectorConstruction::createHousing(std::string name, G4Sipm* sipm) const {
	if (name == "ceramic") {
...
G4VPhysicalVolume* B1DetectorConstruction::DefineVolumes() 
{
	...
	G4Box* scintillator = new G4Box("scintillator", 1. * cm, 1. * cm, scintillator_thickness);
...
void B1DetectorConstruction::ConstructSDandField()
{
	...
	//auto SipmSD= new B1CalorimeterSD("SiPMSD", "AllHitsCollection", 1); // or set fNofLayers to 1?  
	//G4SDManager::GetSDMpointer()->AddNewDetector(SipmSD);
	//SetSensitiveDetector("SiPM_logical", SipmSD);

	// Electric field ----------------------------------------------------------

	fElectricField = new B1ElectricField();
